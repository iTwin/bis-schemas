...
### GridAxisGroupsGridSurfaces

A relationship that maps grid axis to its member surfaces

<u>Naming:</u>

1.  Named as per standards - noting that `GridAxis` groups `GridSurface` instances.

<u>Schema:</u>

```xml
    <ECRelationshipClass typeName="GridAxisGroupsGridSurfaces" modifier="None" strength="embedding" description="maps grid axis to its member surfaces">
